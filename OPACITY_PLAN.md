...
- Optional Multiverse adapter coverage for counter, contended/high-contention counter retries, snapshot reads during retry contention, tiny-set, bank, read-only/read-heavy bank snapshots, direct snapshot transaction bank totals with bounded updater detector behavior, same-address field reuse, shared-array smoke scenarios, a passing tiny direct-snapshot shared-array model check, protocol-respecting Mode2 shared-array smoke coverage, and a larger constant-total shared-array snapshot/updater stress smoke with retry-safe accumulation.
...
The smoke suite includes bounded application coverage for normal read transactions, direct `TX_IS_SNAPSHOT` transaction calls where the checkout exposes them, concurrent updater/read mixes, retry-heavy updater scenarios, and snapshot/updater cases. The shared-array direct-snapshot coverage currently includes a tiny passing model-check case and a larger bounded stress smoke with retry-safe accumulation. The default Mode2 shared-array coverage enters Mode2 through Multiverse's transition protocol in the scenario init phase, runs the snapshot/updater operations as the application-layer workload, and releases Mode2 in the post phase. A separate manual direct-forced Mode2 detector intentionally uses a test-only mode switch so the model checker can find the known snapshot/updater violation on the broken hooked checkout. The optional `third_party/mvcc_tm-mode2-algorithm-fixes.patch` is kept separate from the hook patch, and `tools/check_multiverse_mode2_algorithm_fix.sh` runs the before/after routine while reversing the algorithm patch before exit. Multiverse-specific hook call sites, patching, tests, and examples should stay isolated from the core opacity verifier.
...
- Add optional Multiverse adapter scenarios beyond bounded smoke/application coverage if a target workload needs higher contention, longer runtimes, or public hooks for internal mode transitions beyond the current test-only Mode2 helpers.
- Consider a fuller setup helper for cloning/checking out/applying Multiverse patches. Lightweight validation helpers now exist for the hook patch and for the optional Mode2 algorithm before/after routine.
