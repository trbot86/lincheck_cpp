...
The shared-array snapshot/updater smoke uses a constant-total invariant: updater transactions transfer value between slots while a direct `TX_IS_SNAPSHOT` reader sums the whole array. The tiny model-check variant verifies the hook/opacity path with a small array. The bounded stress variant intentionally expects an `invalid_results` failure when Multiverse exposes an inconsistent direct snapshot result; a 100k-slot version is too large for the default suite and should be treated as an opt-in benchmark shape.
